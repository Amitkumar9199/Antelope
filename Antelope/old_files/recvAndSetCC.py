...
# NOTE: Append the congestion control algorithms (order is importan)
ccNameMap = {0: "cubic",
             1: "bbr",
             2: "westwood",
             3: "illinois"}

ccFileMap = {0: "traindata/models/cubic.pickle",
             1: "traindata/models/bbr.pickle",
             2: "traindata/models/westwood.pickle",
             3: "/usr/src/python/traindata/illinois.pickle"}
...
class OnlineServer:
            ...
            if self.read < self.write:
                ...
                try:
                    if key not in self.flowStaticData:
                        ...
                        print("entering last ACK paket for given port: " + key)
    ...
    def predicCC(self, trainData, key):
        ...
        if (preCC is not None) and (preCC == result or ccArray[-2] != result):
            print("preCC: " + str(preCC), " ccArray: " + str(ccArray))   ####
            return preCC


        preCCMap[key] = result
        ipKey = struct.unpack('!I', socket.inet_aton(data['Destination']))[0]
        ...
        self.changeCong.update_congestion_hash(int(key), int(ipkey_29MSB),
                                               int(ipkey_3LSB), int(result),
                                               int(ipPredic))
        return result
    ...
    def intervalAction(self, countIndex, key):
        ...
        print("[intervalAction]: countIndex: " + str(countIndex) + " rtt: " + str(data['meanRTT']))
        beta = 512
        if countIndex < 9:
    ...
    def bashWriteTrainData(self):
        ...
        for cc in trainDataCCMap.keys():
            if (trainDataCCMap[cc].__len__() > 0):
                fileName = "traindata/"+ccNameMap[cc]+"_output_new.txt"
    ...
    def calReward(self, trainData, rtt):
        ...
        # measure rtt and calculate delay from it
        # aging factor, consider latest delay more weightage
        # fetch why the formula??
        reward = ((trainData['throughput'] - trainData['lost']) * trainData['minRTT']) / (rtt*trainData['max_pacing_rate'])
        return reward
