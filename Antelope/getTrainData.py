...
class OnlineServer:
    ...
    def calTrainData(self, key, preData):
        ...
        print("pacing_rate: ",self.flowStaticData[key]['pacing_rate'][-1])
    ...
    def calReward(self, trainData, rtt):
        print("throughput: "+ str(trainData['throughput']) +" lost: " + str(trainData['lost']) + " meanRTT: " + str(trainData['meanRTT']) + " minRTT: " + str(
            trainData['minRTT']) + " rtt: " + str(rtt) + " max_throughput: " + str(trainData['maxThroughput'])+ " max_pacing_rate: " + str(trainData['max_pacing_rate']))
        reward = ((trainData['throughput'] * 1000 - trainData['lost']) * trainData['minRTT']) / (rtt*trainData['max_pacing_rate'])
        print("REWARD--(throughput-1000*lost)*minrtt/(rtt*max_pacing_rate): ", reward) 
        return reward
